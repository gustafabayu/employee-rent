                ...
                <div class="portlet light">
                    ...
                    <div class="portlet-body">
                                ...
                                <div class="col-md-6">
                                    <div class="btn-group">
                                        <?php  foreach ($almtAir as $set) : ?>
                                            <a id="table_kwh_new" class="btn btn-circle green-haze" href='<?php echo base_url("flow_mtr/addFlow")."?id=".$set->IDBlok; ?>'>
                                                <i class="fa fa-plus"></i> Tambah Data
                                            </a>
                                        <?php endforeach; ?>
                                    </div>
                                    <div class="btn-group">
                                        <a id="table_flow_new" class="btn btn-circle btn-danger" href='<?php echo base_url("flow_mtr/index"); ?>'>
                        ...
                        <table id="table_flow" cellspacing="0" cellpadding="0" class="table table-bordered table-striped table-condensed flip-content flip-scroll">
                            ...
                            <tbody>
                                <?php $no = 0; foreach ($getFlow as $setF){?>
                                    <tr>
                                        <td><?php echo ++$no; ?></td>
                                        <td><strong><?php $this->load->model('m_flow_mtr'); $a = $this->m_flow_mtr->selectAlamat($setF->IDMess);
                                            foreach ($a as $set): echo $set->Nama; endforeach; ?></strong></td>
                                        <td><strong><?php echo $setF->Nomor; ?></strong></td>
                                        <td><?php echo $setF->Merk; ?></td>
                                        <td><?php echo $setF->Spesifikasi; ?></td>
                                        <td><?php echo $setF->TglPasang; ?></td>
                                        <td><strong><?php $this->load->model('m_flow_mtr'); $b = $this->m_flow_mtr->getAir($setF->TipePakai);
                                            foreach ($b as $setA): echo $setA->Tipe; endforeach; ?></strong></td>
                                        <td><?php echo $setF->Ket; ?></td>
                                        <td>
                                            <a href="<?php echo site_url('flow_mtr/editFlow')."?id=".$setF->IDFlowMtr; ?>" class="btn btn-circle blue">
...
<script>
    var table = $('#table_flow');
        ...
        var tableWrapper = $('#table_flow_wrapper'); // datatable creates the table wrapper by adding with id {your_table_jd}_wrapper
